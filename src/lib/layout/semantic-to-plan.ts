...
export function semanticToDrawingPlan(diagram: SemanticDiagram, region: Region): DrawingPlan {
  ...
  for (const node of layout.nodes) {
    ...
    let y = sy(r.y) + 30;
    const x = sx(r.x) + PAD;
    if (node.title) { commands.push({ type: "text", x, y, text: node.title, fontSize: 24, color: "#0f172a" }); y += 32; }
    if (node.description) { commands.push({ type: "text", x, y, text: node.description, fontSize: 18, color: "#334155" }); y += 31; }
    for (const item of node.items ?? []) { commands.push({ type: "text", x, y, text: `• ${item}`, fontSize: 18, color: "#334155" }); y += 31; }
  }
...
function polylineCommands(points: Point[], label?: string): DrawingCommand[] {
  ...
  if (label) { const mid = points[Math.floor(points.length / 2)]; out.push({ type: "text", x: mid.x + 8, y: mid.y - 8, text: label, fontSize: 16, color: "#475569" }); }
  return out;
}
