...
export function semanticToDrawingPlan(diagram: SemanticDiagram): DrawingPlan {
  ...
  const pad = Math.max(6, PAD * Math.min(1, scale)); const gap = Math.max(4, HEADER_GAP * Math.min(1, scale));
  ...
  for (const group of layout.groups) {
    ...
    if (group.title) commands.push({ type: "text", x: sx(group.rect.x) + pad, y: sy(group.rect.y) + Math.max(20, 28 * Math.min(1, scale)), text: group.title, fontSize: font(22), color: "#475569" });
  }
  for (const edge of layout.edges) { const points = edge.points.map((p) => ({ x: sx(p.x), y: sy(p.y) })); commands.push(...polylineCommands(points, edge.label, font(16))); }
  for (const node of layout.nodes) {
    ...
    let y = sy(r.y) + Math.max(20, 30 * Math.min(1, scale)); const x = sx(r.x) + pad;
    if (node.title) { const size = font(24); commands.push({ type: "text", x, y, text: node.title, fontSize: size, color: "#0f172a" }); y += size * 1.35 + gap; }
    if (node.description) { const size = font(18); commands.push({ type: "text", x, y, text: node.description, fontSize: size, color: "#334155" }); y += size * 1.35 + gap; }
    for (const item of node.items ?? []) { const size = font(18); commands.push({ type: "text", x, y, text: `• ${item}`, fontSize: size, color: "#334155" }); y += size * 1.35 + gap; }
  }
...
function polylineCommands(points: Point[], label?: string, labelSize = 16): DrawingCommand[] {
  ...
  for (let i = 0; i < points.length - 1; i++) { const last = i === points.length - 2; out.push({ type: last ? "arrow" : "line", x1: points[i].x, y1: points[i].y, x2: points[i + 1].x, y2: points[i + 1].y, strokeWidth: 3, color: "#64748b" }); }
