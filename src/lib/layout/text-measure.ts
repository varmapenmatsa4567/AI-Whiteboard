...
export function measureText(text: string, style: TextStyle, maxWidth = Infinity): TextMeasure {
  ...
  const fontSize = Math.max(6, style.fontSize);
  ...
  if (typeof document !== "undefined") {
    ...
    if (ctx) {
      ctx.font = `${style.fontWeight ?? 400} ${fontSize}px ${style.fontFamily ?? "Arial, sans-serif"}`;
      return measureWithCanvas(ctx, value, fontSize * lineHeight, maxWidth);
    }
  }
...
function measureWithCanvas(ctx: CanvasRenderingContext2D, text: string, lineHeightPx: number, maxWidth: number): TextMeasure {
  ...
  const width = Math.min(maxWidth, Math.max(...lines.map((line) => ctx.measureText(line).width), 0));
  return { width, height: lines.length * lineHeightPx, lines: lines.length };
}
...
export function measureCardSize(node: { title?: string; description?: string; items?: string[]; width?: number; height?: number }, options: CardSizeOptions = {}): { width: number; height: number } {
  ...
  const titleStyle = options.titleStyle ?? { fontSize: 20, fontWeight: 700 };
  const bodyStyle = options.bodyStyle ?? { fontSize: 15, lineHeight: 1.35 };
  const content = [node.title, node.description, ...(node.items ?? [])].filter((v): v is string => Boolean(v?.trim()));

  const preferred = Math.max(o.minWidth, Math.min(o.maxWidth, node.width ?? 240));
  const inner = Math.max(40, preferred - o.horizontalPadding * 2);
  let height = o.verticalPadding * 2;
  let width = 0;
  let hasContent = false;

  if (node.title?.trim()) {
    const m = measureText(node.title, titleStyle, inner);
    height += m.height;
    width = Math.max(width, m.width);
    hasContent = true;
  }
  if (node.description?.trim()) {
    if (node.title?.trim()) height += o.gap;
    const m = measureText(node.description, bodyStyle, inner);
    height += m.height;
    width = Math.max(width, m.width);
    hasContent = true;
  }
  if ((node.items ?? []).length) {
    if (node.title?.trim() || node.description?.trim()) height += o.gap;
    for (const item of node.items ?? []) {
      const m = measureText(`• ${item}`, bodyStyle, inner);
      height += m.height;
      width = Math.max(width, m.width);
    }
    hasContent = true;
