...
export function layoutDiagram(diagram: SemanticDiagram, options: LayoutOptions = {}): LayoutResult {
  const originX = options.originX ?? 80; const originY = options.originY ?? 80;
  ...
  const depth = assignDepths(diagram); const layers = new Map<number, LaidOutNode[]>();
  for (const node of nodes) { const d = depth.get(node.id) ?? 0; const layer = layers.get(d) ?? []; layer.push(node); layers.set(d, layer); }
  const direction = diagram.direction === "top-to-bottom" ? "tb" : "lr";
  const orderedLayers = [...layers.keys()].sort((a, b) => a - b);
  let cursor = direction === "lr" ? originX : originY; const crossCursor = direction === "lr" ? originY : originX;
  for (const key of orderedLayers) {
    const layer = layers.get(key) ?? []; let layerSize = 0; for (const node of layer) layerSize += direction === "lr" ? node.rect.height : node.rect.width; layerSize += Math.max(0, layer.length - 1) * NODE_GAP;
    let cross = crossCursor;
    for (const node of layer) {
      if (direction === "lr") { node.rect.x = cursor; node.rect.y = cross; cross += node.rect.height + NODE_GAP; }
      else { node.rect.x = cross; node.rect.y = cursor; cross += node.rect.width + NODE_GAP; }
    }
...
function assignDepths(diagram: SemanticDiagram): Map<string, number> {
  const depth = new Map<string, number>(); const incoming = new Map<string, number>();
  ...
  while (queue.length) {
    const id = queue.shift()!; remaining.delete(id); const d = depth.get(id) ?? 0;
    for (const edge of diagram.edges.filter((e) => e.source === id)) { depth.set(edge.target, Math.max(depth.get(edge.target) ?? 0, d + 1)); if (![...queue].includes(edge.target)) queue.push(edge.target); }
  }
...
function relaxCollisions(nodes: LaidOutNode[]): void {
  for (let pass = 0; pass < 12; pass++) {
    ...
    for (let i = 0; i < nodes.length; i++) for (let j = i + 1; j < nodes.length; j++) {
      ...
      if (Math.abs(dx) >= Math.abs(dy)) { const shift = (a.width + b.width) / 2 + NODE_GAP - Math.abs(dx); const sign = dx >= 0 ? 1 : -1; a.x += (shift * sign) / 2; b.x -= (shift * sign) / 2; }
      else { const shift = (a.height + b.height) / 2 + NODE_GAP - Math.abs(dy); const sign = dy >= 0 ? 1 : -1; a.y += (shift * sign) / 2; b.y -= (shift * sign) / 2; }
      moved = true;
    }
    if (!moved) break;
  }
