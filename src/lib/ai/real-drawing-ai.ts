...
export interface RealDrawingAIInit {
  apiKey?: string;
  baseUrl?: string;
  model?: string;
  jsonMode?: boolean;
 ...
 * Swappable through AI_API_KEY / AI_BASE_URL / AI_MODEL.
 */
export class RealDrawingAI implements DrawingAI {
  ...
  async generateDrawing(request: DrawingRequestContext): Promise<DrawingPlan> {
    ...
    // Ask the provider for output. Some models are unreliable at producing
    // valid JSON, so retry a few times (with a corrective nudge) before giving up.
    const maxAttempts = 3;
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      const systemPrompt =
        attempt === 0 ? SYSTEM_PROMPT : `${SYSTEM_PROMPT}\n\n${REPAIR_SYSTEM_PROMPT}`;
      ...
      try {
        return parseDrawingPlan(content).plan;
      } catch (e) {
        lastRaw = e instanceof AIParseError ? e.raw : content;
